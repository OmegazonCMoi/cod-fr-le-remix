...
const Videos = () => {
    ...
    const [selectedLinks, setSelectedLinks] = useState<{ [key: string]: string }>({}); // To track selected links for each video
    ...
    const handleSelectChange = (videoId: string, value: string) => {
        const updatedLink =
            value === "1" ? videos.find(v => v.id === videoId)?.link :
                value === "2" ? videos.find(v => v.id === videoId)?.steamLink :
                    value === "3" ? videos.find(v => v.id === videoId)?.cheatsLink :
                        '';
    ...
    return (
        <div id="videos" className="min-h-screen flex flex-col">
            ...
            <div className="flex-grow grid place-items-center">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 items-center justify-items-center">
                    {videos.map((video) => (
                        <Card key={video.id} className="w-[350px] h-full">
                            <CardHeader>
                            ...
                            <CardFooter className="flex gap-4 justify-end">
                                ...
                                <Link href={selectedLinks[video.id] || '#'}>
                                    <Button>
                                        Go to Link
                                        <svg
